...
def load_candidates(status: str = "new", limit: int = 0) -> list[dict]:
    ...
    with get_driver() as driver:
        with driver.transaction(TYPEDB_DATABASE, TransactionType.READ) as tx:
            query = f'''match $c isa jhunt-candidate, has jhunt-candidate-status "{status}";
                fetch {{
                    "id": $c.id,
                    "name": $c.name,
...
def update_candidate_status(candidate_id: str, new_status: str,
    ...
    """Update candidate status and optionally jhunt-relevance-score and jhunt-triage-reason in TypeDB."""
    with get_driver() as driver:
        with driver.transaction(TYPEDB_DATABASE, TransactionType.WRITE) as tx:
            tx.query(f'''match
                $c isa jhunt-candidate, has id "{candidate_id}", has jhunt-candidate-status $s;
                delete $c has $s;''').resolve()
            tx.query(f'''match
                $c isa jhunt-candidate, has id "{candidate_id}";
                insert $c has jhunt-candidate-status "{new_status}";''').resolve()
            if relevance_score is not None:
                # Delete old score if exists, then insert new one
                tx.query(f'''match
                    $c isa jhunt-candidate, has id "{candidate_id}", has jhunt-relevance-score $rs;
                    delete $c has $rs;''').resolve()
                tx.query(f'''match
                    $c isa jhunt-candidate, has id "{candidate_id}";
                    insert $c has jhunt-relevance-score {relevance_score};''').resolve()
            if triage_reason is not None:
                # Delete old reason if exists, then insert new one
                tx.query(f'''match
                    $c isa jhunt-candidate, has id "{candidate_id}", has jhunt-triage-reason $tr;
                    delete $c has $tr;''').resolve()
                tx.query(f'''match
                    $c isa jhunt-candidate, has id "{candidate_id}";
                    insert $c has jhunt-triage-reason "{escape_string(triage_reason)}";''').resolve()
            tx.commit()
